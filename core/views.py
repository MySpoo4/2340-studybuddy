...
def swipe_view(request):
    ...
    swiped_users_ids = Swipe.objects.filter(swiper=current_user).values_list(
        "swiped_id", flat=True
    )

    # Users who have already matched with the current user
    matched_users_ids_raw = Match.objects.filter(
        Q(user1=current_user) | Q(user2=current_user)
    ).values_list("user1_id", "user2_id")
    # Flatten the list of tuples and remove duplicates
    matched_users_ids = {
        item
        for sublist in matched_users_ids_raw
        for item in sublist
    ...
    potential_matches_qs = (
        User.objects.exclude(id__in=exclude_ids)
        .select_related("profile")
        .prefetch_related("profile__courses", "profile__study_preferences")
    )

    # Get the current user's courses and preferences for comparison
    current_user_courses = set(
        current_user.profile.courses.values_list("id", flat=True)
    )
    current_user_preferences = set(
        current_user.profile.study_preferences.values_list("id", flat=True)
    )
    ...
    for user in potential_matches_qs:
        ...
        user_preferences = set(
            user.profile.study_preferences.values_list("id", flat=True)
        )

        common_courses_count = len(current_user_courses.intersection(user_courses))
        common_preferences_count = len(
            current_user_preferences.intersection(user_preferences)
        )
    ...
    likers_ids = Swipe.objects.filter(swiped=current_user, liked=True).values_list(
        "swiper_id", flat=True
    )

    # Find IDs of users the current user has already swiped on
    already_swiped_ids = Swipe.objects.filter(swiper=current_user).values_list(
        "swiped_id", flat=True
    )
...
def matches_view(request):
    ...
    Match.objects.filter(user1=current_user, user1_notified=False).update(
        user1_notified=True
    )
    Match.objects.filter(user2=current_user, user2_notified=False).update(
        user2_notified=True
    )
    ...
    for match in user_matches:
        ...
        if (
            ...
            and current_user_profile.longitude
            and other_user.profile.latitude
            and other_user.profile.longitude
        ):
            distance = haversine(
                current_user_profile.latitude,
                current_user_profile.longitude,
                other_user.profile.latitude,
                other_user.profile.longitude,
            )
...
def swipe_action_view(request):
    if request.method == "POST":
        ...
        if swiper == swiped_user:
            return JsonResponse(
                {"status": "error", "message": "Cannot swipe on yourself"}, status=400
            )

        # Create or update the swipe
        swipe, created = Swipe.objects.update_or_create(
            swiper=swiper, swiped=swiped_user, defaults={"liked": liked}
        )
        ...
        if liked:
            ...
            try:
                reverse_swipe = Swipe.objects.get(
                    swiper=swiped_user, swiped=swiper, liked=True
                )
                # It's a match!
                ...
                )  # This will now have the new notified fields as False by default
...
def likes_you_view(request):
    ...
    users_who_liked_you = (
        User.objects.filter(id__in=users_to_show_ids)
        .select_related("profile")
        .prefetch_related("profile__courses", "profile__study_preferences")
    )
...
def inbox_view(request):
    ...
    current_user = request.user
    matches = (
        Match.objects.filter(Q(user1=current_user) | Q(user2=current_user))
        .select_related("user1__profile", "user2__profile")
    ...
    for match in matches:
        ...
        matches_with_last_message.append({"match": match, "last_message": last_message})

    # Sort conversations by the timestamp of the last message, descending.
    matches_with_last_message.sort(
        key=lambda x: (
            x["last_message"].timestamp if x["last_message"] else x["match"].timestamp
        ),
        reverse=True,
    )

    return render(
        request, "core/inbox.html", {"matches_data": matches_with_last_message}
    )
...
def chat_room_view(request, match_id):
    ...
    Message.objects.filter(match=match, is_read=False).exclude(
        sender=current_user
    ).update(is_read=True)
...
def fetch_messages_view(request, match_id):
    ...
    get_object_or_404(
        Match, Q(id=match_id) & (Q(user1=current_user) | Q(user2=current_user))
    )

    messages = (
        Message.objects.filter(match_id=match_id)
...
def study_sessions_list(request):
    ...
    created_sessions = StudySession.objects.filter(creator=current_user).order_by(
        "start_time"
    )

    # Sessions where the user is a participant
    participant_sessions = (
        StudySession.objects.filter(participants=current_user)
        ...
        if sync_to_calendar:
            ...
            if profile.google_calendar_refresh_token:
                ...
                if event_id:
                    ...
                    session.save()
                    messages.success(
                        request, "Study session created and synced to Google Calendar!"
                    )
                else:
                    messages.warning(
                        request,
                        "Study session created, but failed to sync to Google Calendar.",
                    )
            else:
                messages.warning(
                    request,
                    "Study session created. Please connect Google Calendar to sync.",
                )
        else:
    ...
    if request.method == "POST":
        ...
        if sync_to_calendar:
            ...
            if profile.google_calendar_refresh_token:
                ...
                if event_id:
                    messages.success(
                        request, "Study session updated and synced to Google Calendar!"
                    )
                else:
                    messages.warning(
                        request,
                        "Study session updated, but failed to sync to Google Calendar.",
                    )
            else:
                messages.warning(
                    request,
                    "Study session updated. Please connect Google Calendar to sync.",
                )
        else:
...
def google_calendar_callback(request):
    ...
    if "error" in request.GET:
        ...
        print(
            f"ERROR: Google Calendar authorization failed. Error: '{error}'. Description: {error_description}"
        )
    ...
    try:
        ...
        messages.error(
            request,
            "Failed to connect to Google Calendar. There might be a configuration issue.",
        )
        return redirect("core:study_sessions_list")
...
def sync_session_to_google_calendar(session, profile):
    ...
    try:
        ...
        event = {
            ...
            or f"Study session at {session.location}",
            "start": {
        ...
        print(
            f'ERROR: Error details: {error.error_details if hasattr(error, "error_details") else "N/A"}'
        )
        return None
...
def update_google_calendar_event(session, profile):
    ...
    try:
        ...
        event = (
            service.events()
            .get(calendarId="primary", eventId=session.google_calendar_event_id)
            .execute()
        )
        ...
        event["description"] = (
            session.description or f"Study session at {session.location}"
        )
        event["start"] = {
...
def delete_google_calendar_event(session, profile):
    ...
    try:
        ...
        service.events().delete(
            calendarId="primary", eventId=session.google_calendar_event_id
        ).execute()
        return True
    except HttpError as error:
        print(f"An error occurred: {error}")
